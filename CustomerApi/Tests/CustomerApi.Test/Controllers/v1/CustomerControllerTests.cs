...
namespace CustomerApi.Test.Controllers.v1
{
    public class CustomerControllerTests
    {
        ...
        public CustomerControllerTests()
        {
            ...
            // setup do Moq por exemplo
            A.CallTo(() => mapper.Map<Customer>(A<Customer>.Ignored)).Returns(customer);
            A.CallTo(() => _mediator.Send(A<CreateCustomerCommand>.Ignored, default)).Returns(customer);
            A.CallTo(() => _mediator.Send(A<UpdateCustomerCommand>.Ignored, default)).Returns(customer);
        }
        ...
        {
            // arrange
            A.CallTo(() => _mediator.Send(A<CreateCustomerCommand>.Ignored, default))
            ...
            // assert
            (result.Result as StatusCodeResult)?
                .StatusCode
                .Should()
        ...
        {
            A.CallTo(() => _mediator.Send(A<UpdateCustomerCommand>._, default))
                .Throws(new Exception(exceptionMessage));


            var result = await _testee.Customer(_id, _updateCustomerModel);
